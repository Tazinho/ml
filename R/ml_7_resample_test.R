#' Create samples of test data for varying model validation needs
#'
...
#' from first call. If you instead want to add an additional resampling, set \code{add}
#' to \code{TRUE}.
#' 
#' @param seed The seed (integer) used to create the random samples.
#' 
#' @return A list containing the train and test sets as well as a .resample_test
...
#'
#' @examples
#' ml_7_resample_test(data = iris)
#'
#' @seealso \href{https://github.com/Tazinho/ml}{ml on github}.
#'
#' @export
#'
ml_7_resample_test <- function(data_sets_list,
                               group_by = NULL,
                               sample_train = NULL,
                               replace = FALSE,
                               output = "list",
